...
export default class GradesDetails extends React.Component {
  ...
  findNextCourse = direction => {
    ...
    var allSemesterCourses = this.props.navigation.state.params.semester
      .courses;
    ...
    this.setState({
      course: allSemesterCourses[newIndex],
  ...
  handleAnimation = newWidths => {
    ...
    Animated.parallel(
      this.state.gradeIndicators.map(item => {
        return timing(item.width, {
          toValue: Math.floor(Math.random() * (280 - 5 + 1)) + 5
        });
      })
    ).start();
    /**
      ...
      <Container style={Styles.ContainerStyle}>
        ...
        <Content style={styles.container}>
          <ListItem>
            ...
            >
              <Button transparent onPress={() => this.findNextCourse(-1)}>
                <Icon name="ios-arrow-back" />
              </Button>

              <Text>
                {" "}
                {course.course_title.length > 25
                  ? course.course_title.substring(0, 25) + "."
                  : course.course_title}
              </Text>
          ...
          <ListItem>
            <Text>
              Grade: {course.grade} ( {course.note} )
            </Text>
          </ListItem>
          ...
          <View style={{ marginVertical: 15 }}>
            {this.state.gradeIndicators.map(grade => (
              <View style={styles.item} key={grade.grade}>
                    ...
                    <Animated.View
                      style={[
                        ...
                        styles.points,
                        {
                          width: grade.width,
                          backgroundColor:
                            grade.grade === course.grade ? "blue" : "red"
                        }
                      ]}
                    />
                  }
